...
/* Return a random number uniformly distributed between [min, max] */
float uniform_omp(float min, float max, unsigned seedp)
{
    float normalized; 
    normalized = (float)rand_r(&seedp)/(float)RAND_MAX;
    return (min + normalized * (max - min));
}
...
int pso_get_best_fitness_omp(swarm_t *swarm, int num_threads)
{
    ...
    /* each thread will hold different value for  g and best_fitness */
    /* i is unique for each thread */
    int * tid_g = (int*)malloc(num_threads * sizeof(int) );
    float * tid_best_fitness = (float*)malloc(num_threads * sizeof(float) );

    /* initiate best_fit to INF for each thread */
    for (i =0 ; i < num_threads; i++){
       tid_best_fitness[i] = INFINITY;
    }

    int tid;
#pragma omp parallel num_threads(num_threads) shared(tid_g,tid_best_fitness,swarm) private(i,particle,tid)
    {
    tid = omp_get_thread_num();
#pragma omp for
    /* tid_g contains index of particle associated to each thread */
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        if (particle->fitness < tid_best_fitness[tid]) { 
            tid_best_fitness[tid] = particle->fitness;
            tid_g[tid] = i;
        }
    }
    ...
    float best_fitness = INFINITY;
    g = -1;
    /* particle associated with each thread */
    /* find best fit among all threads */
    for (i =0; i < num_threads; i++){
        particle = &swarm->particle[tid_g[i]];
        if (particle->fitness < best_fitness){
           best_fitness = particle->fitness;
           g = tid_g[i];
        }
    ...
    free(tid_g); free(tid_best_fitness);
                  ...
                  float xmin, float xmax, int num_threads)
{
    ...
    /* seed for rand_r() in uniform_omp() */
    unsigned seedp;
    int tid;

#pragma omp parallel num_threads(num_threads) shared(swarm,function,xmin,xmax,dim) private(particle,i,j,seedp)
    {
    tid = omp_get_thread_num();
    seedp = time(NULL) + tid;
#pragma omp for
    for (i = 0; i < swarm->num_particles; i++) {
        ...
        particle->x = (float *)malloc(dim * sizeof(float));
        for (j = 0; j < dim; j++)
           particle->x[j] = uniform_omp(xmin, xmax,seedp);
...
#pragma omp parallel for num_threads(num_threads) shared(swarm) private(particle)
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        particle->g = g;
    }
