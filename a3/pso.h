...
float uniform_omp(float, float, unsigned int); //updated
...
int pso_solve_omp(char *, swarm_t *, float, float, int,int); //updated

void pso_free(swarm_t *);

int pso_get_best_fitness(swarm_t *); 
int pso_get_best_fitness_omp(swarm_t *, int num_threads); //updated

int optimize_gold(char *, int, int, float, float, int);
int optimize_using_omp(char *, int, int, float, float, int, int); //updated
